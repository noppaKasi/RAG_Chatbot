...
function ChatApp() {
  ...
  const sendMessage = () => {
    ...
    if (input.trim()) {
      ...
      const message = JSON.stringify({ input, web_search: webSearch });
   ...
   * Depending on whether it's a user message or a server message, it's displayed differently (you on the right, else AI on the left).
  ...
  const renderMessageAsMarkdown = (msg) => {
    ...
    const displayMessage = isUserMessage ? `<b>You:</b><br />${msg.substring(5)}` : `<b>AI:</b><br />${msg}`;
